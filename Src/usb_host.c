...
volatile int cnttttt = 0;
volatile int cntttttb = 0;
...
#define SE_0   {DM_GPIO_Port->BSRR = snd[2];cpuDelay(TIME_DELAY);}
...
//#define DM_SPin (1<<(DM_PIN-8))
//#define DP_SPin (1<<(DP_PIN-8))
...
#define DMT_Pin  (1 << DM_PIN)
#define DPT_Pin  (1 << DP_PIN)

#define SET_I (DM_GPIO_Port->MODER  &= ~(0x3u*DMT_Pin*DMT_Pin|0x3u*DPT_Pin*DPT_Pin))
#define SET_O (DM_GPIO_Port->MODER  |=  (0x1u*DMT_Pin*DMT_Pin|0x1u*DPT_Pin*DPT_Pin))
...
//Req rq;
#define LOCAL_DEF_BUFF_SIZE  64
#define MICRO_DEF_BUFF_SIZE  8
typedef struct
{
int 			isValid;	
int 			selfNum;
...
int parse_received_NRZI_buffer()
{
	...
	for(int i = 1;i<received_NRZI_buffer_bytesCnt;i++)
	{
		...
		if( tm < 10  || (smb == 0) || (smb == (M_ONE + P_ONE)))
		//if( tm < 10  || (smb == 0))
		{
			//terr+=tm<4?tm : 4;
			terr+=tm;
